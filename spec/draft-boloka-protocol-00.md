---
title: "The Boloka Protocol"
subtitle: "Specification — Draft 0.2 (draft-boloka-protocol-00)"
author: "Boloka Protocol Editors"
date: "July 2026"
abstract: |
...
This document is Draft 0.2 of the Boloka protocol specification. It is a
...
:   A small, signed, immutable statement in the envelope format of
    Section 5.1. All application data in Boloka is records.
...
confused with a signature produced for any other Boloka context (future
...
Every signature and hash in Boloka names its algorithm by registry id.
...
identifier, or one of the Boloka-specific values
...
* Changes follow a lightweight RFC process. Kinds graduate into the
  ...
  conformance suite.
* "Boloka-compliant" is a certifiable trademark claim, carrying the
...
Boloka's contribution is the combination, plus the two layers all of
...
3. **Signature input.** The signature is over the domain-separated id
   (`"boloka:record:v1" || id`), not over raw envelope bytes
   (Section 5.4).
...
1. **Nostr interoperability.** With the clean-room kernel locked
   ...
   Boloka records into Nostr events and Blossom blob conventions for
