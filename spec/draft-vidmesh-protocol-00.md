---
title: "The Vidmesh Protocol"
subtitle: "Specification — Draft 0.2 (draft-vidmesh-protocol-00)"
author: "Vidmesh Protocol Editors"
date: "July 2026"
abstract: |
  Vidmesh is a protocol for publishing, distributing, and discussing video
...
This document is Draft 0.2 of the Vidmesh protocol specification. It is a
...
:   A small, signed, immutable statement in the envelope format of
    Section 5.1. All application data in Vidmesh is records.
...
confused with a signature produced for any other Vidmesh context (future
...
Every signature and hash in Vidmesh names its algorithm by registry id.
...
identifier, or one of the Vidmesh-specific values
...
* Changes follow a lightweight RFC process. Kinds graduate into the
  ...
  conformance suite.
* "Vidmesh-compliant" is a certifiable trademark claim, carrying the
...
Vidmesh's contribution is the combination, plus the two layers all of
...
3. **Signature input.** The signature is over the domain-separated id
   (`"vidmesh:record:v1" || id`), not over raw envelope bytes
   (Section 5.4).
...
1. **Nostr interoperability.** With the clean-room kernel locked
   ...
   Vidmesh records into Nostr events and Blossom blob conventions for
