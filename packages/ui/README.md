...
Shared React components for every Vidmesh gateway frontend. This is the
...
`@vidmesh/kernel`, compare the derived id to the requested id) and maps
...
1. Add `"@vidmesh/ui": "workspace:*"` (or a published version) and
...
these `.tsx` files itself). `pnpm --filter @vidmesh/ui lint` runs
