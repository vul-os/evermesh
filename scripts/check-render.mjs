...
const TITLE_MUST_MATCH = {
  'index.html': /^Evermesh\b/,
  // docs.html re-titles itself per chapter — "006 · Relay — Evermesh docs" —
  // so the assertion anchors on the suffix the shell always writes rather than
  // on a prefix it only has before the router runs. It still refuses a decoy:
  // the self-test serves another product's <title> and this rejects it.
  'docs.html': /\bEvermesh docs$/,
};
...
const MATRIX_TARGETS = ['index.html', 'docs.html', 'docs.html#draft-evermesh-protocol-00'];
