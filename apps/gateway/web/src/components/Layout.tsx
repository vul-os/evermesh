import { MoonIcon, NowPlayingBar, QueueProvider, SearchIcon, SunIcon } from "@evermesh/ui";
 ...
 *
 * Wrapped in `QueueProvider` (@evermesh/ui) so the audio queue survives
 * route changes: `NowPlayingBar` is mounted here, once, as a sibling of
 * `<Outlet>` rather than inside a route — a card several routes away
 * that adds a track to the queue and this bar share the one queue
 * instance the provider holds, the same way `useMe`/`useTheme` are
 * shared app-wide rather than re-fetched per route.
 */
