...
respond to a takedown notice. Vidmesh's design principle is that
...
1. **Upload pipeline, pre-publish.** Before a freshly uploaded blob is
   ...
   ([VIDMESH_BUILD_PLAN.md](../../../VIDMESH_BUILD_PLAN.md) §9, upload
...
- **NOT-FOR-PRODUCTION** — running real user traffic, real uploads, or
  ...
  **not covered by the Vidmesh trademark program** — a deployment on
  the stub matcher is not entitled to represent itself as a compliant
  Vidmesh reference gateway, regardless of how the rest of the stack is
  configured.
