## Agent prompt: build the Boloka monorepo from zero

You are an expert protocol engineer and full-stack developer. Your task is to build
**Boloka** — a decentralized video protocol and its reference implementations — as a
...
A companion document, `BOLOKA_SPEC_PROPOSAL.md`, is provided alongside this plan and
...
Boloka is "many gateways, one substrate" video:
...
│   └── 011-threat-model.md
├── crates/
│   ├── boloka-kernel/        ← the permanent library
│   ├── boloka-relay/         ← axum websocket+http relay binary
│   ├── boloka-wasm/          ← wasm-bindgen wrapper over kernel
│   └── boloka-node/          ← Tauri 2 scaffold (Phase 8)
├── packages/
...
│   └── site/                  ← boloka.org landing page, static, see §14
└── tools/
...
| 2 | `boloka-kernel` | all §6 APIs implemented; unit tests + test vectors pass; no `unsafe` |
| 3 | `boloka-wasm` + `kernel-ts` | browser + node can verify/create records identically to Rust tests |
| 4 | `boloka-relay` | relay passes conformance suite; two relays gossip records |
...
Transcribe and expand `BOLOKA_SPEC_PROPOSAL.md` into the `spec/` folder. Rules:
...
## 6. Phase 2 — `boloka-kernel` public API
...
`boloka-wasm` exposes the kernel via wasm-bindgen. `packages/kernel-ts` wraps it in
...
wordmark "boloka" in lowercase, set in an open-license geometric sans (embed as
