...
//! (`boloka_relay` and its modules) so it can be unit-tested without
//! a running process.
...
use boloka_relay::{blobs, config::RelayConfig, gossip, info, store::Store, sync, AppState};
...
async fn main() -> anyhow::Result<()> {
    ...
    let config_path = std::env::args()
        .nth(1)
        .or_else(|| std::env::var("BOLOKA_RELAY_CONFIG").ok())
        .map(PathBuf::from);
    ...
    let config = match config_path {
        ...
        None => {
            tracing::warn!(
                "no config path given (argv[1] or BOLOKA_RELAY_CONFIG unset); using defaults"
            );
            RelayConfig::default()
        }
    };
    ...
    tracing::info!("boloka-relay listening on {listen_addr}");
