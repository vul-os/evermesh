...
API (`boloka_kernel::Record`, `ChunkTree`) that is specified but not
...
```sh
cargo run -p boloka-relay -- path/to/relay.json
# or
BOLOKA_RELAY_CONFIG=path/to/relay.json cargo run -p boloka-relay
```
...
depends on `boloka_kernel::Record`/`ChunkTree`, which are specified
