...
The Boloka node app: a background desktop app (Tauri 2) that pins and
...
- `src/pinning.rs` is the **real v1 design surface**, written now so the
  ...
  `boloka-kernel`'s `BlobId`/`RecordId`, with every method already
...
```sh
cargo check -p boloka-node
cargo test -p boloka-node
```
...
| Path | Purpose |
|---|---|
| `Cargo.toml` | crate manifest: `tauri`, `tauri-build`, `serde`/`serde_json`, `boloka-kernel` |
| `build.rs` | `tauri_build::build()` — bundle glue from `tauri.conf.json` |
| `tauri.conf.json` | Tauri 2 config: identifier `org.boloka.node`, static `ui/` frontend, window config |
