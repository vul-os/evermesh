...
//!
//! Evermesh independently arrived at the same substrate shape as DMTAP-PUB §22
...
//!
//! This module is the **§22 path**, sitting **beside** the native evermesh
...
//! only the **bridge** — conversions between evermesh's kernel types and §22's,
//! and the §24-video-profile framing of a evermesh record.
//!
//! # What the bridge does and does not claim
//!
//! Conversions here are **representational, not cryptographic**. Converting a
//! evermesh [`crate::BlobId`] into a §22 [`ContentId`] re-frames an address that
//! was computed by evermesh's rules; it does **not** make a evermesh chunk-tree
...
/// Evermesh deliberately owns none of this code.
...
//
// Evermesh addresses are bare 32 bytes, rendered `b3-256:<hex>` in text form.
...
/// Frame a bare evermesh 32-byte digest as a §22 multihash [`ContentId`]
/// (`0x1e ‖ digest`).
...
/// evermesh representation and MUST NOT be silently truncated).
...
/// Frame a evermesh [`BlobId`] as a §22 [`ContentId`].
///
...
/// Recover a evermesh [`BlobId`] from a §22 [`ContentId`], if it is a
...
/// Reinterpret a evermesh [`Keypair`] as a §22 [`IdentityKey`].
///
...
/// convergence is tractable: an existing evermesh author can publish §22 objects
...
/// The §22.2.2 plaintext chunk addresses for a blob, using evermesh's
/// [`CHUNK_SIZE`] (1 MiB) split: `h_i = 0x1e ‖ BLAKE3-256(plaintext_i)`.
///
/// Note what is hashed: the **chunk bytes**, producing an *address*. §22's tree
/// then hashes those addresses. Evermesh's tree hashes the chunk bytes directly
...
/// evermesh's 1 MiB [`CHUNK_SIZE`].
///
/// Returns `None` for the empty blob: §22 requires `n ≥ 1` chunks, whereas
/// evermesh's [`crate::ChunkTree`] tolerates the empty tree with no root. That
...
pub mod divergence {
    ...
    /// structure* for every `n` (evermesh's odd-node promotion and §22's RFC-6962
    ...
    /// separation.
    ///
    /// Returns `(evermesh_root, pub_root)` for a blob, or `None` if empty. These
    ...
    /// Per-chunk: what evermesh **stored** versus what §22 **needs**, for one chunk.
    ///
    /// Returns `(evermesh_leaf, pub_chunk_address)` where
    /// `evermesh_leaf = BLAKE3(0x00 ‖ chunk)` ([`crate::blob::leaf_hash`]) and
    ...
    /// bytes." That was never true of evermesh's format: evermesh folds a `0x00` leaf
...
/// `meta` key under which a bridged announce carries the originating evermesh
...
pub const META_KEY_EVERMESH_KIND: &str = "evermesh.kind";

/// `meta` key carrying the originating evermesh record id, so a bridged announce
/// remains traceable to the native record it was derived from.
pub const META_KEY_EVERMESH_RECORD: &str = "evermesh.record";

/// Build the §24-profile `meta` block describing a native evermesh [`Record`].
///
...
pub fn meta_for_record(record: &Record) -> Vec<(String, Cv)> {
    vec![
        (META_KEY_EVERMESH_KIND.to_string(), Cv::U64(record.kind())),
        (
            META_KEY_EVERMESH_RECORD.to_string(),
...
/// by `dmtap-core` itself — evermesh does not construct the preimage.
...
/// Bridge a native evermesh [`Record`] and its blob to a signed §22 announce:
...
// ── Feed construction (§22.4) — the primitive evermesh does not have ──────────
...
///
/// **This has no evermesh counterpart.** Evermesh has no signed feed head at all,
...
mod tests {
    ...
    /// Pins the §24.14-item-4 erratum: evermesh's retained per-chunk digest is
    /// NOT the digest §22 needs, so migration cannot skip re-reading the media.
    #[test]
    fn stored_evermesh_leaf_is_not_the_pub_chunk_address() {
        ...
        // ...and what evermesh stored is a different digest entirely.
        assert_ne!(
            vid_leaf,
    ...
    #[test]
    fn non_blake3_content_id_has_no_evermesh_form() {
    ...
    fn empty_blob_has_no_pub_manifest() {
        ...
        // ...while evermesh tolerates the empty tree. A phase-2 migration item.
    ...
    fn anti_rollback_rejects_a_lower_seq_head() {
        // The capability evermesh has no counterpart for at all.
        let k = kp();
    ...
    fn record_meta_carries_kind_and_record_id() {
        ...
        assert_eq!(meta[0].0, META_KEY_EVERMESH_KIND);
        assert_eq!(meta[1].0, META_KEY_EVERMESH_RECORD);
    }
}
