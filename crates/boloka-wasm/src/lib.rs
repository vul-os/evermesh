//! wasm-bindgen bindings over `boloka-kernel`.
//!
...
use boloka_kernel::{codec, kinds, Identity, IdentityId, Keypair, Record, RecordId};
use wasm_bindgen::prelude::*;

fn js_err(e: boloka_kernel::Error) -> JsError {
...
pub fn build_record(
    ...
    for entry in refs_arr {
        ...
        refs.push(boloka_kernel::Ref {
            ref_type,
            hash: take32(hash, "ref hash")?,
        });
    }
    let body = codec::from_json(body_json).map_err(js_err)?;
    let record = boloka_kernel::RecordBuilder::new(kind as u64)
...
pub fn hash_blob(bytes: &[u8]) -> Vec<u8> {
    boloka_kernel::blob::hash_blob(bytes).as_bytes().to_vec()
}

/// Sign a rendition derivation statement (DMTAP §24.4.4, superseding
/// spec 004 §3.1's video-only text). The statement construction lives
/// in the kernel so every runtime signs identical bytes. Returns the
/// 64-byte signature for the manifest's `derivation_sig` field.
///
/// `width`/`height` are optional (`undefined` on the JS side): both
/// present means this rendition carries a video track, both absent
/// means audio-only (DMTAP §24.4.2). The kernel encodes an absent
/// dimension as CBOR `null` at its fixed position in the signed
/// preimage — never `0`, never a shortened array (§24.4.4, VID-17). A
/// caller passing exactly one of `width`/`height` gets the kernel's
/// both-or-neither rejection, surfaced as a `JsError`.
...
pub fn sign_derivation(
    ...
    if width.is_some() != height.is_some() {
        return Err(JsError::new(
            "width and height must be both present or both absent",
        ));
    }
    let original = boloka_kernel::BlobId(take32(original, "original blob id")?);
    let rendition_id = boloka_kernel::BlobId(take32(rendition, "rendition blob id")?);
    let statement = kinds::derivation_statement(
        &original,
        &rendition_id,
        codec,
        width.map(|w| w as u64),
...
pub fn verify_chunk(
    ...
    let siblings: Vec<[u8; 32]> = proof
        ...
        .collect();
    boloka_kernel::blob::verify_chunk(&root, n_chunks as usize, index as usize, chunk, &siblings)
        .map_err(js_err)
}
...
impl BlobHasher {
    ...
    pub fn update(&mut self, bytes: &[u8]) -> Result<(), JsError> {
        ...
        let mut rest = bytes;
        let chunk_size = boloka_kernel::blob::CHUNK_SIZE;
        while !rest.is_empty() {
            ...
            if self.pending.len() == chunk_size {
                self.leaves
                    .push(boloka_kernel::blob::leaf_hash(&self.pending));
                self.pending.clear();
            }
        }
        Ok(())
        ...
        self.root = if level.is_empty() {
            ...
            while level.len() > 1 {
                ...
                for pair in level.chunks(2) {
                    match pair {
                        [l, r] => next.push(boloka_kernel::blob::node_hash(l, r)),
                        [odd] => next.push(*odd),
                        _ => {}
                    }
                }
...
mod tests {
    ...
    fn blob_hasher_matches_kernel() {
        let data = vec![7u8; boloka_kernel::blob::CHUNK_SIZE + 123];
        ...
        assert_eq!(
            h.id_hex().unwrap(),
            boloka_kernel::blob::hash_blob(&data).to_hex()
        );
        let tree = boloka_kernel::ChunkTree::from_bytes(&data);
