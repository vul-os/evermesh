...
use vidmesh_kernel::identity::{AlgKey, Identity};
use vidmesh_kernel::record::{Record, RecordBuilder, Ref, SIG_ALG_ED25519};
use vidmesh_kernel::{BlobId, IdentityId, Keypair, RecordId};
...
proptest! {
    ...
    fn bundle_round_trip(
        ...
        for content in &blob_contents {
            let id = vidmesh_kernel::blob::hash_blob(content);
        ...
        let mut out = Vec::new();
        vidmesh_kernel::Bundle::export(&records, &expected_blobs, &mut out).unwrap();
        let result = vidmesh_kernel::Bundle::import(&out[..]).unwrap();
