...
use super::{
    ...
    required_nonempty_text, required_text, required_u64, text_array, text_field, validate_license,
    KIND_ROTATION,
};
...
/// The source or a derived encoding of a video (spec 004 §2).
///
...
pub struct Media {
    ...
    /// RFC 6381 codecs string, e.g. `"av01.0.08M.08"`.
    pub codec: String,
...
impl Media {
    ...
    fn entries(&self) -> Vec<(Value, Value)> {
        let mut e = vec![
            ...
            (Value::Text("width".into()), Value::Uint(self.width)),
            (Value::Text("height".into()), Value::Uint(self.height)),
        ];
    ...
    fn parse(v: &Value) -> Result<Media> {
        ...
        let width = required_u64(v, "width", "media: width required")?;
        let height = required_u64(v, "height", "media: height required")?;
...
/// A caption/subtitle track (spec 004 §2).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Caption {
...
pub const DERIVATION_SIG_PREFIX: &[u8] = b"vidmesh:derivation:v1";

/// Build the derivation statement a rendition producer signs (spec 004
/// §3.1): `canonical_cbor([original.blob, rendition.blob, codec, width,
/// height, bitrate])`.
...
/// Verify a rendition's derivation signature against `original` (spec
/// 004 §3.1): recompute the statement, hash it, and check
/// `derivation_sig` under `produced_by`'s key.
///
