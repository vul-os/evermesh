...
//!
//! Boloka independently arrived at the same substrate shape as DMTAP-PUB §22
...
//!
//! This module is the **§22 path**, sitting **beside** the native boloka
...
//! only the **bridge** — conversions between boloka's kernel types and §22's,
//! and the §24-video-profile framing of a boloka record.
//!
//! # What the bridge does and does not claim
//!
//! Conversions here are **representational, not cryptographic**. Converting a
//! boloka [`crate::BlobId`] into a §22 [`ContentId`] re-frames an address that
//! was computed by boloka's rules; it does **not** make a boloka chunk-tree
...
/// Boloka deliberately owns none of this code.
...
//
// Boloka addresses are bare 32 bytes, rendered `b3-256:<hex>` in text form.
...
/// Frame a bare boloka 32-byte digest as a §22 multihash [`ContentId`]
/// (`0x1e ‖ digest`).
...
/// boloka representation and MUST NOT be silently truncated).
...
/// Frame a boloka [`BlobId`] as a §22 [`ContentId`].
///
...
/// Frame a boloka [`RecordId`] as a §22 [`ContentId`].
///
/// Representational only: a boloka record id hashes the envelope **without**
...
/// incompatible rules — this conversion is for referencing boloka objects from
/// §22 `meta`, never for claiming a §22 anchor.
pub fn record_id_to_content_id(id: &RecordId) -> ContentId {
    content_id_from_digest(id.as_bytes())
}

/// Recover a boloka [`BlobId`] from a §22 [`ContentId`], if it is a
...
/// Reinterpret a boloka [`Keypair`] as a §22 [`IdentityKey`].
///
...
/// convergence is tractable: an existing boloka author can publish §22 objects
...
/// The §22.2.2 plaintext chunk addresses for a blob, using boloka's
/// [`CHUNK_SIZE`] (1 MiB) split: `h_i = 0x1e ‖ BLAKE3-256(plaintext_i)`.
///
/// Note what is hashed: the **chunk bytes**, producing an *address*. §22's tree
/// then hashes those addresses. Boloka's tree hashes the chunk bytes directly
...
/// boloka's 1 MiB [`CHUNK_SIZE`].
///
/// Returns `None` for the empty blob: §22 requires `n ≥ 1` chunks, whereas
/// boloka's [`crate::ChunkTree`] tolerates the empty tree with no root. That
...
pub mod divergence {
    ...
    /// structure* for every `n` (boloka's odd-node promotion and §22's RFC-6962
    ...
    /// separation.
    ///
    /// Returns `(boloka_root, pub_root)` for a blob, or `None` if empty. These
    ...
    /// Per-chunk: what boloka **stored** versus what §22 **needs**, for one chunk.
    ///
    /// Returns `(boloka_leaf, pub_chunk_address)` where
    /// `boloka_leaf = BLAKE3(0x00 ‖ chunk)` ([`crate::blob::leaf_hash`]) and
    /// `pub_chunk_address = 0x1e ‖ BLAKE3(chunk)` ([`chunk_hash`]).
    ///
    /// # Why this exists — corpus anchor for a corrected spec erratum
    ///
    /// An earlier revision of DMTAP §24.14 item 4 claimed that on migration "the
    /// chunk leaf hashes are identical (bare-chunk BLAKE3) … so re-derivation is a
    /// tree recompute over the existing chunk hashes, not a re-read of the media
    /// bytes." That was never true of boloka's format: boloka folds a `0x00` leaf
    /// tag *inside* the hash, so what it persisted per chunk is
    /// `BLAKE3(0x00 ‖ chunk)`, while §22's `h_i` digest is `BLAKE3(chunk)` —
    /// different preimages, different values, asserted unequal by this module's
    /// tests.
    ///
    /// §24.17 erratum **C-01** has since corrected §24.14 item 4 to state this
    /// plainly: migrating a blob to §22 requires **re-reading and re-hashing every
    /// stored media byte**, an I/O-bound cost proportional to total stored bytes,
    /// never a cheap metadata-only tree recompute over retained digests. This
    /// module's `chunk_digests_for`/`roots_for` and their tests are the
    /// corpus-anchored evidence C-01 cites for that correction — the finding here
    /// predates, and does not merely restate, the now-corrected spec text. See
    /// `docs/DMTAP-CONVERGENCE.md`.
...
/// `meta` key under which a bridged announce carries the originating boloka
...
pub const META_KEY_BOLOKA_KIND: &str = "boloka.kind";

/// `meta` key carrying the originating boloka record id, so a bridged announce
/// remains traceable to the native record it was derived from.
pub const META_KEY_BOLOKA_RECORD: &str = "boloka.record";

/// Build the §24-profile `meta` block describing a native boloka [`Record`].
///
...
pub fn meta_for_record(record: &Record) -> Vec<(String, Cv)> {
    vec![
        (META_KEY_BOLOKA_KIND.to_string(), Cv::U64(record.kind())),
        (
            META_KEY_BOLOKA_RECORD.to_string(),
...
/// by `dmtap-core` itself — boloka does not construct the preimage.
...
/// Bridge a native boloka [`Record`] and its blob to a signed §22 announce:
...
// ── Feed construction (§22.4) — the primitive boloka does not have ──────────
...
///
/// **This has no boloka counterpart.** Boloka has no signed feed head at all,
...
mod tests {
    ...
    /// Pins the §24.14-item-4 erratum: boloka's retained per-chunk digest is
    /// NOT the digest §22 needs, so migration cannot skip re-reading the media.
    #[test]
    fn stored_boloka_leaf_is_not_the_pub_chunk_address() {
        ...
        // ...and what boloka stored is a different digest entirely.
        assert_ne!(
            vid_leaf,
    ...
    #[test]
    fn non_blake3_content_id_has_no_boloka_form() {
    ...
    fn native_and_pub_roots_differ_for_every_chunk_count() {
        for n in 1usize..=9 {
            ...
            assert_ne!(
                ...
                "boloka and §22 roots must NOT coincide (n={n}); \
    ...
    fn empty_blob_has_no_pub_manifest() {
        ...
        // ...while boloka tolerates the empty tree. A phase-2 migration item.
    ...
    fn anti_rollback_rejects_a_lower_seq_head() {
        // The capability boloka has no counterpart for at all.
        let k = kp();
    ...
    fn record_meta_carries_kind_and_record_id() {
        ...
        assert_eq!(meta[0].0, META_KEY_BOLOKA_KIND);
        assert_eq!(meta[1].0, META_KEY_BOLOKA_RECORD);
    }
}
