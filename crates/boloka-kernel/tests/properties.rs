...
use boloka_kernel::identity::{AlgKey, Identity};
use boloka_kernel::record::{Record, RecordBuilder, Ref, SIG_ALG_ED25519};
use boloka_kernel::{BlobId, IdentityId, Keypair, RecordId};
...
proptest! {
    ...
    fn bundle_round_trip(
        ...
        for content in &blob_contents {
            let id = boloka_kernel::blob::hash_blob(content);
        ...
        let mut out = Vec::new();
        boloka_kernel::Bundle::export(&records, &expected_blobs, &mut out).unwrap();
        let result = boloka_kernel::Bundle::import(&out[..]).unwrap();
